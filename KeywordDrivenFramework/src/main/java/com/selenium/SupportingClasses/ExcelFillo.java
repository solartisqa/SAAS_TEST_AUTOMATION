...
public class ExcelFillo {
	...
	// ***********************update
	// query**************************************************************************************************************
	public void updateRecordset(String tableName, String ColumnName, String Value, String PrimaryKey)
			throws FilloException {
		// strQuery = "update "+tableName+" set "+ColumnName+"='"+Value+"' where
		// "+PrimaryKey+"='"+recordset.getField(PrimaryKey)+"'";
		// strQuery = "update Sheet1 set Policy Number='001' where Seq No='1'";
		System.out.println("UPDATE " + tableName + " SET " + ColumnName + "='" + Value + "' WHERE " + PrimaryKey + "='"
				+ recordset.getField(PrimaryKey) + "'");
		connection.executeUpdate("UPDATE " + tableName + " SET " + ColumnName + "='" + Value + "' WHERE " + PrimaryKey
				+ "='" + recordset.getField(PrimaryKey) + "'");
		// +'"+recordset.getField(PrimaryKey)+"');
	}
