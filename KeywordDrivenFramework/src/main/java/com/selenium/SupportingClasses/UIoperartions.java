...
public class UIoperartions extends browserLaunching {
	...
	public void perform(PropertiesHandle config, String p, String operation, String objectType, String value,
		...
		try {
			 ...
			 * 
			 * this.click(p, objectType); long start = System.currentTimeMillis();
			 * assertTrue(driver.getPageSource().contains(value)); long end =
			 * System.currentTimeMillis(); //long seconds =
			 * TimeUnit.MILLISECONDS.toSeconds(end-start);
			 * System.out.println("Round trip response time = " + (end-start) + " Millis");
			 * System.out.println("Round trip response time = " + (((end-start)/1000)) +
			 * " Soconds"); break;
			 */
			// ------------------------------------------------------------------------------------------------------------------------------------------------------
			...
			case "CHECKRESPONSETIME":
				...
				if (wdriver.getPageSource().contains(value)) {
					...
					// outputData.put(SecondsOutputcolum,
					// String.valueOf(pageLoadTime_Seconds)+"."+pageLoadTime_Seconds1);
					outputData.put(dbcolumn_name, String.valueOf(pageLoadTime_ms) + " , "
							+ String.valueOf(pageLoadTime_Seconds) + "." + pageLoadTime_Seconds1);
				} else {
			...
			this.perform(config, p, operation, objectType, value, dbcolumn_name, dataFlag, InputData, outputData,
					waitingTime);
		}
	...
	protected void clickVisibleElement(String p, String objectType)
	...
	protected String getInputValue(String dataFlag, LinkedHashMap<String, String> InputData, String value,
			String dbcolumn_name) throws SQLException {
	...
	private void selectFromelementlistByValue(String p, String objectType, String inputValue)
	...
	private void selectFromelementlistByText(String p, String objectType, String inputValue)
			throws InterruptedException {
		List<WebElement> elementList = wdriver.findElements(this.getObject(p, objectType));
		String[] inputlist = inputValue.split(";");
		for (int i = 0; i < inputlist.length; i++) {
