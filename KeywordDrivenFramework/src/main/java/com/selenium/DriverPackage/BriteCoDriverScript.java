...
public class BriteCoDriverScript extends UIoperartions implements UIScriptsInterface {
	...
	// script========================================================================================
	public void executeTestScript(LinkedHashMap<String, String> InputData, LinkedHashMap<String, String> outputData)
			throws SQLException, IOException, InterruptedException, AWTException, DatabaseException {
		...
		while (objectTestScript.has_next_row()) {
			...
			if (objectTestScript.read_data(objectTestScript.get_rownumber(), 8).toString().equals("enabled")
					&& ConditionsChecking.ConditionReading(conditions, InputData)) {
				...
				if (objectTestScript.read_data(objectTestScript.get_rownumber(), 11).toString().equals("loop")) {
					...
					System.out.println("-----------------"
							+ InputData.get(objectTestScript.read_data(objectTestScript.get_rownumber(), 5)));
					if (str1 == null) {
						...
						while ((objectTestScript.read_data(objectTestScript.get_rownumber(), 11).toString()
								.equals("loop"))) {
				...
				if (loopFlag) {
					...
					for (int i = 0; i < n; i++) {
						for (int j = 0; j < l; j++) {
							if (status[j].equals("enabled")
									&& ConditionsChecking.ConditionReading(condition[j], InputData)) {

								this.perform(configFile, PropertyString[j], actionKeyword[j], ObjectType[j], value[j],
										dbcolumnNmae[j], dataProvidingFlag[j], InputData, outputData, waitingTime[j]);
							}
						}
					}
				...
				if (!loopFlag) {
					...
					this.perform(configFile, PropertyString1, actionKeyword1, ObjectType1, value1, dbcolumnNmae1,
							dataProvidingFlag1, InputData, outputData, waitingTime1);
	...
	// results========================================================================================================
	/*
	 * public void comparisonScript(DatabaseOperation objectInput,DatabaseOperation
	 * objectOutput) throws SQLException { objectComparisonScript.set_rownumber(1);
	 * while(objectComparisonScript.has_next_row()) { String
	 * conditions=objectComparisonScript.read_data(objectComparisonScript.
	 * get_rownumber(),4);
	 * if(objectComparisonScript.read_data(objectComparisonScript.get_rownumber(),3)
	 * .toString().equals("Y")&& this.ConditionReading(conditions, objectInput)) {
	 * String expectedcolumn =
	 * objectComparisonScript.read_data(objectComparisonScript.get_rownumber(),0);
	 * String actualColumn =
	 * objectComparisonScript.read_data(objectComparisonScript.get_rownumber(),1);
	 * String StatusColumn =
	 * objectComparisonScript.read_data(objectComparisonScript.get_rownumber(),2);
	 * String expectedValue =
	 * (objectOutput.read_data(expectedcolumn)).replace("$","").replace(",","");
	 * String actualValue =
	 * (objectOutput.read_data(actualColumn)).replace("$","").replace(",","");
	 * 
	 * if(expectedValue.equals(actualValue)) {
	 * objectOutput.write_data(StatusColumn,"Pass"); } else {
	...
	// ==================================Function to compare the
	// results========================================================================================================
	public LinkedHashMap<String, String> CompareFunction(LinkedHashMap<String, String> outputrow,
			LinkedHashMap<String, String> comparisonrow) {
		try {
	...
	// ======================================PRIVATE FUNCTION FOR SUPPORTING
	// COMPARISON FUNCTION=======================================
	...
	protected void analyse(LinkedHashMap<String, String> Conditiontablerow, LinkedHashMap<String, String> outputrow)
